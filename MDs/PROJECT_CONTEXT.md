...
- 여러 캘린더를 쓸 때는 첫 줄에 `[별칭] 일정`을 쓰고, 다음 줄에 등록 내용 또는 조회 명령을 쓴다. 예: `Tom 일정` 줄 다음에 `5.20 연차 = 하루전날`, 또는 `Tom 일정` 줄 다음에 `오늘일정`.
- **날짜**
...
- **옵션(예시)**
  ...
  - `오전 N시` / `오후 N시` / `당일 오전 N시` / `당일 오후 N시` → 일정 당일 지정 시각 알림. 시간 일정은 시작 시각 기준 몇 분 전 reminder로 변환된다.
  ...
  - 종일·기간 일정에서 `오전 9시`가 아닌 당일 특정 시각은 Google Calendar 종일 reminder UI/API 표현 제약이 있으므로 시간 일정에서 사용하는 것을 권장한다.
...
- 캘린더 별칭 헤더와 함께 명령어를 쓰면 해당 캘린더를 조회한다. 예: `Tom 일정` 줄 다음에 `오늘일정`, `다음주 일정`, `6.20 일정`.
